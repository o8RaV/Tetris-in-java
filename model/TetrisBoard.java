...
public class TetrisBoard implements Serializable{
    ...
    public int placementHeight(TetrisPiece piece, int x) {
        ...
        for (int i = 0; i < piece.getWidth(); i++)
            if (getColumnHeight(x + i) > temp)
                temp = getColumnHeight(x + i);
        return temp;
    }
    ...
    public int placePiece(TetrisPiece piece, int x, int y) {
        if (!committed)
            throw new RuntimeException("Board not committed");
        committed = false;
        backupGrid();
        int result = ADD_OK;
        ...
        for (TetrisPoint point : piece.getBody()) {
            ...
            int Y = point.y + y;
            tetrisGrid[X][Y] = true;
            if (this.colCounts[X] < Y + 1)
                this.colCounts[X] = Y + 1;
            this.rowCounts[Y]++;
            if (this.rowCounts[Y] == width)
    ...
    private void clearrowhelper(int start){
