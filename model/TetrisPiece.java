...
public class TetrisPiece implements Serializable {
    ...
    */
    ...
    public TetrisPiece(TetrisPoint[] points) {
        ...
        Arrays.fill(temp, height);
        for(TetrisPoint point : points){
            if(temp[point.x] > point.y){
    ...
    public boolean equals(Object obj) {
        try {
            TetrisPiece obj1 = (TetrisPiece) obj;
            return Arrays.equals(Arrays.stream(obj1.body).sorted().toArray(), Arrays.stream(this.body).sorted().toArray());
        }
        catch (Exception e) {
            return false;
        }
    ...
    public static TetrisPiece makeFastRotations(TetrisPiece root) {
        ...
        while (!curr.computeNextRotation().equals(root)){
    ...
    public TetrisPiece computeNextRotation() {
        TetrisPoint[] temp = new TetrisPoint[this.body.length];
        for(int i =0; i <temp.length; i++){
            temp[i] = new TetrisPoint(this.height -1 - this.body[i].y, this.body[i].x);
        }
        return new TetrisPiece(temp);
    }
